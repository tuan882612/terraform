...
additional information that constraints the possible range of the final value/
...
* `5` and `6` represent the lower and upper bounds respectively of the length
  of a collection value. The valueof both is an integer representing an
...
but considering refinements may allow a more precise answer.
...
When encoding an unknown value without any requirements, always use the older
